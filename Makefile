...
.PHONY: get-books create-book update-book delete-books get-collections create-collection update-collection delete-collection create-books-collection delete-books-collection
...
.PHONY: create-book update-book delete-book
...
delete-books:
	...
	SERVER_CONTAINER=$$(docker ps | grep server-bm | awk '{print $$1}'); \
	ID="$(if $(ID),--id=$(ID),)"; \
	docker exec -it $$SERVER_CONTAINER /bin/sh -c "./cli-client --action=delete_books $$ID"

.PHONY: get-collections create-collection update-collection delete-collection
...
create-books-collection:
	...
	SERVER_CONTAINER=$$(docker ps | grep server-bm | awk '{print $$1}'); \
	docker exec -it $$SERVER_CONTAINER /bin/sh -c "./cli-client --action=create_books_collection --cid=1 --book_ids=1,2,3"

delete-books-collection:
	@echo "Running delete-books-collection target"; \
	SERVER_CONTAINER=$$(docker ps | grep server-bm | awk '{print $$1}'); \
	docker exec -it $$SERVER_CONTAINER /bin/sh -c "./cli-client --action=delete_books_collection --cid=1 --book_ids=1,2,3"
